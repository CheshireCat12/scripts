...
#SBATCH --output=/storage/homefs/ag21k209/neo_slurms/classification_gk_SP_on_spectral_%A_%a.out
#SBATCH --array=1-680

param_store=./arguments_gk_on_spectral_full.txt
...
srun python main.py --root_dataset $SCRATCH/data/$dataset_save/spectral/$clustering_algo/$merging_method/red_fact$reduction/dimensions$dimension --graph_kernel SP --n_cores 10 --save_predictions --folder_results $HOME/graph_library/results/gk_SP_classification/$dataset_save/spectral/$clustering_algo/$merging_method/red_fact$reduction/dimensions$dimension -v
